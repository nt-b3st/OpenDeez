...
class Echo(Adw.Application):
    ...
    def _show_home(self,chart,releases,pls):
        ...
        if name:
            welcome=Gtk.Label(label=f"Bienvenue {name} sur OpenDeez")
            welcome.set_css_classes(["title-2"]); welcome.set_halign(Gtk.Align.START)
            welcome.set_margin_start(16); welcome.set_margin_top(16); welcome.set_margin_bottom(4)
            self.home_box.append(welcome)
        if pls:
